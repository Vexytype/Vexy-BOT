const { ChannelType, EmbedBuilder, ActionRowBuilder, ButtonBuilder, StringSelectMenuBuilder } = require('discord.js');
...
const convertTime = (time, timezone = "America/Sao_Paulo") => {
    const [hour, minute] = time.split(':').map(Number);

    const localTime = DateTime.local().setZone(timezone).set({ hour, minute });
...
function AutoLockChannel(client, automaticos) {
    ...
    for (const guildId in automaticos) {
        ...
        channels.forEach(async (channelId) => {
            ...
            const jobName = `${jobPrefix}${channelId}`;

            schedule.scheduleJob(jobName, lockTimeConvert, async () => {
                ...
                try {
                    ...
                    const embed = new EmbedBuilder()
                        ...
                        .setDescription(`${EMOJI.vx6 == null ? `` : `<:${EMOJI.vx6.name}:${EMOJI.vx6.id}>`} — ${cumprimento()} a todos.\nEste canal foi trancado pelo sistema.`)
                        .setFooter(
                            { text: `Reabertura do canal - ${unlockTime}`, iconURL: "https://cdn.discordapp.com/emojis/1265111283103961219.webp?size=96&quality=lossless" }
                        )
            ...
            schedule.scheduleJob(unlockTimeConvert, async () => {
                ...
                try {
                    ...
                    const embed123 = new EmbedBuilder()
                        .setColor(General.get('oficecolor.main'))
                        .setDescription(`${EMOJI.vx6 == null ? `` : `<:${EMOJI.vx6.name}:${EMOJI.vx6.id}>`} — ${cumprimento()} a todos.\nO canal foi liberado para uso novamente!`)
                        .setFooter(
...
function AutoAnnounce(client, ConfigAnnc) {
    ...
    for (const key in Produtos) {
        ...
        if (!Data || Object.keys(Data.Campos).length === 0 || !Data.messageid || Data.messageid.length === 0) {
            continue;
        }

        const TimeRepost = convertTime(ConfigAnnc);
        const jobName = `${jobPrefix}${Data.Config.name}`;

        schedule.scheduleJob(jobName, TimeRepost, async () => {
            try {
                SendAutoAnnc(client, Data.Config.name);
            } catch (error) {
                console.log(error);
            }
        });
...
async function SendAutoAnnc(client, produtin) {
    ...
    let item = await products.get(`proodutos.${produtin}.messageid`)
    ...
    if (CampoQnty == 1) {
        ...
        for (const iterator in item) {
            ...
            try {
                ...
                console.log('Autoannounce:' + error)
            }
        }
...
module.exports = {
    ...
    run: async (client) => {
        ...
        setInterval(() => {
            ...
            if (JSON.stringify(newAutoRepost) !== JSON.stringify(ConfigAnnc)) {

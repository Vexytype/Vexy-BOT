...
async function notifyStock(quanti, produtin, CampoSelect, interaction) {
    ...
    const channel = interaction.client.channels.cache.get(`${Valor3.channelid}`);
...
async function configCoupons(produtin, CampoSelect, interaction, client) {3
    const Valor1 = await products.get(`proodutos.${produtin}.Campos.${CampoSelect}.name`)
    const Valor = await products.get(`proodutos.${produtin}.Campos.${CampoSelect}.cupom`)

    const count = Object.keys(Valor).length || null;

    const embed = new EmbedBuilder()
        .setAuthor({ name: `Gerenciar Cupons`, iconURL: "https://cdn.discordapp.com/emojis/1291805653412872235.webp?size=96&quality=lossless" })
        .setDescription(`- **Campo**: ${Valor1}\n\n- Total de Cupons: ${count == null ? `0` : count}`)
        .setColor(General.get("oficecolor.main"))
        .setFooter(
            { text: interaction.guild.name, iconURL: interaction.guild.iconURL({ dynamic: true }) }
        )
    ...
    interaction.update({
        ...
        components: [
            new ActionRowBuilder()
                .addComponents(
                    new ButtonBuilder()
                        .setURL('https://github.com/Vexytype')
                        .setLabel('Github')
                        .setStyle(5)
                        .setEmoji('1298662377473966087'),
                    new ButtonBuilder()
                        .setURL('https://vexyapiv1.squareweb.app/v1/convite')
                        .setLabel("Vexy Community")
                        .setStyle(5)
